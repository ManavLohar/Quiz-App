...
const ConfirmationModel = () => {
  ...
  const questions = useSelector(
    (state: RootState) => state.quizSlice.questions
  );
  const questionId = useSelector(
    (state: RootState) => state.quizSlice.questionId
  );
  const filterQuestion = questions.find((item) => item._id === questionId);
  ...
  return (
    <div className="fixed top-0 left-0 h-screen w-full flex justify-center items-center bg-[#00000099]">
      <div className="relative flex flex-col gap-2 justify-between w-[400px] h-fit bg-slate-300 rounded-md">
        ...
        <div className="px-4">
          <p className="text-lg">Q. {filterQuestion?.question}</p>
